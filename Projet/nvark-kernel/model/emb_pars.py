...
def SSR_parameters(TRAIN_x_l_, T_min, filter_data=True, filter_scale=0.1, plot=False):
    TRAIN_x_l = TRAIN_x_l_.copy()
    if filter_data:
        filtered_train = filter_MTS(TRAIN_x_l, filter_scale=filter_scale, plot=plot)
    else:
