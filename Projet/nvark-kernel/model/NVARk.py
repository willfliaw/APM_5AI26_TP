...
class NVARk(BaseEstimator):
    def __init__(
        ...
        if self.verbose_lvl == 2:
            print("--NVARk model--")
            print(
                f"\tinit params = [k:{self.k}, n:{self.n}, s:{self.s}, d_red:{self.n_dim}, lamb:{self.lamb}, gamma_mult:{self.gamma_mult}]"
            )
    ...
    def sample_indices(self, DATA_x_l):
        ...
        if type(DATA_x_l) == list:
            self.D = DATA_x_l[0].shape[1]
        # elif type(DATA_x_l)==pd.DataFrame: self.D = len(self.X.columns)
        else:
            raise RuntimeError(
                "Invalid input type. Must be list of 2D arrays [[N], T, D]"
            )

        # sample indices
        n_dim_add = (
            self.n_dim - self.D
        )  # calculate the number of dimensions to add to reach n_dim
        if n_dim_add <= 0:
            warnings.warn(
                'The dimensionality of the input is larger than the maximum allowed dimension for the embedding. \
            ...
            i_linear = list(
                range(self.D * (self.k + 1))
            )  # indices for lagged dimensions + input series
            if self.n > 1:
                i_nonlinear = list(
                    itertools.combinations_with_replacement(
                        range(len(i_linear)), self.n
                    )
                )  # indices for polynomial combinations of all linear terms
            else:
                i_nonlinear = []
            ...
            self.sampled_nonlin_indices = self.maybe_sample_index(
                n_dim_add, i_nonlinear
            )
    ...
    def compute_embedding(self, DATA_x_l, indices=None):
        ...
        """
        if self.verbose_lvl == 2:
            print("\tNVAR embeddings computation...")
        # to store the embedding states
        if self.verbose_lvl == 2:
            print("\t\tinput shape: \t", utils.print_shape(DATA_x_l))
        N = len(DATA_x_l)
        ...
        for i in range(N):
            ...
            for k in range(1, self.k + 1):
                ...
                linear_terms[i] = np.concatenate(
                    (linear_terms[i], apply_lag(DATA_x_l[i], lag)), axis=1
                )
            ...
            for index in self.sampled_indices:
                R_nvar[i] = self.cat_dimension(
                    index, R_nvar[i], lagged_terms[i], linear_terms[i]
                )
        ...
        if self.verbose_lvl == 2:
            print("\t\tR_nvar shape: \t", utils.print_shape(R_nvar))
        return R_nvar
    ...
    def cat_dimension(self, index, block, lagged_term, linear_term):
        ...
        if type(index) == int:
            block = np.concatenate(
                (block, lagged_term[:, index].reshape(-1, 1)), axis=1
            )
        # quadratic terms
        if type(index) == tuple and self.n == 2:
            block = np.concatenate(
                (
                    block,
                    (linear_term[:, index[0]] * linear_term[:, index[1]]).reshape(
                        -1, 1
                    ),
                ),
                axis=1,
    ...
    def linear_readout(self, R_nvar, thr=10**-20):
            ...
            for i in range(N):
                ...
                if R_nvar[i].shape[0] > 1:
                    ...
                    self._ridge_embedding = Ridge(
                        alpha=lamb, fit_intercept=True, solver=self.solver
                    )
                    self._ridge_embedding.fit(
                        R_nvar[i][0:-1, :], R_nvar[i][1:, :]
                    )  # fit to next embedding state
                    coeff_tr.append(self._ridge_embedding.coef_.ravel())
                    biases_tr.append(self._ridge_embedding.intercept_.ravel())
                else:
                    coeff_nans = np.empty((D, D)).ravel()
                    coeff_nans[:] = np.nan
                    biases_nans = np.empty(D).ravel()
                    biases_nans[:] = np.nan
        ...
        if self.verbose_lvl == 2:
            print("\t\trep shape: \t\t", input_repr.shape, " -- 2D array")
        return input_repr
    ...
    def rbf_function(self, tr_rep, te_rep=None, mode="tr-tr"):
        ...
        if mode == "tr-tr":
            ...
            # Kernel
            if self.RBF_gamma == 0:
                self.RBF_gamma = 1  # fix division by zero
    ...
    def compute_Ktetr(self, test, train=None):
        ...
        if self.theta_repr_tr is None:
            if train is None:
                raise ValueError(
                    'train input must be given if "compute_Ktrtr" has not been called before'
                )
            _ = self.sample_indices(train)
            self.R_nvar_tr = self.compute_embedding(train)
            self.theta_repr_tr = self.linear_readout(self.R_nvar_tr)
            self.RBF_gamma = self.gamma_mult * np.median(
                squareform(pdist(self.theta_repr_tr, metric="euclidean"))
            )
    ...
    def compute_Ktete(self, test=None, train=None):
        ...
        if self.theta_repr_te is None:
            if test is None:
                raise ValueError(
                    'test input must be given if "compute_Ktetr" has not been called before'
                )
            self.R_nvar_te = self.compute_embedding(test)
            self.theta_repr_te = self.linear_readout(self.R_nvar_te)
    ...
    def optimize_params(
        ...
        """
        params = {
            "k": k_list if k_list is not None else [self.k],
        ...
        if self.verbose_lvl == 1:
            print(
                f"\tNVARk CV optimization with n_folds={n_folds} and val_size={val_size}"
            )
            print(f"\tgrid = {params}")
