...
def print_info(dataset_name, TRAIN, TEST, y=None):
    ...
    for key, dataset in traintest.items():
        ...
        info[key]["equalL"], info[key]["T_min"], info[key]["T_max"] = different_lengths(
            dataset
        )
        info[key]["D"] = len(dataset.columns)
        info[key]["nans"] = dataset_has_nans(dataset)

        line_info = "{:<37}  {:<8} {:<14} {:<8} {:<8} {:<8}".format(
            key,
            info[key]["N"],
    ...
    for d in range(D):
        ...
        for i in range(N_train):
            long_series = pd.concat(
                (long_series, pd.Series(train_red.iloc[i])), ignore_index=True
            )
...
def zero_padding(_train, _test, T_max):
    ...
    for i in range(N_train):
        ...
        if T_diff > 0:
            for d in range(D):
                series = pd.concat(
                    (train.iloc[i, d], pd.Series(np.zeros(T_diff))), ignore_index=True
                )
    ...
    for i in range(N_test):
        ...
        if T_diff > 0:
            for d in range(D):
                series = pd.concat(
                    (test.iloc[i, d], pd.Series(np.zeros(T_diff))), ignore_index=True
                )
...
def preprocessing(
    ...
    if dataset_name == "CharacterTrajectories":
        TRAIN_x, TEST_x, TRAIN_y_raw, TEST_y_raw = clean(
            TRAIN_x, TEST_x, TRAIN_y_raw, TEST_y_raw
        )
    # Interpolate
    ...
    if prepr_option == "zero_padding":
        if (
            not info[f"{dataset_name} train"]["equalL"]
            or info[f"{dataset_name} test"]["equalL"]
        ):
