...
def pdSeriesDataFrame_to_listOfnpArray(dataset):
    list_of_numpy = [
        pdSeries_to_npArray_singleMTS(dataset.iloc[i, :])
...
def print_shape(nested_array):
    try:
        ...
        if all(elem == lengths[0] for elem in lengths):
            T = f"{nested_array[0].shape[0]}"
        else:
            T = f"{max(lengths)}*"
        dims = [nested_array[i].shape[1] for i in range(N)]
        if all(elem == dims[0] for elem in dims):
            D = f"{nested_array[0].shape[1]}"
        else:
            D = f"{max(dims)}*"
